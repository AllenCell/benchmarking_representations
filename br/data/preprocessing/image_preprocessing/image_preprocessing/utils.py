...
from variance_preprocessing.zarr_writer import OmeZarrWriter
...
def write_ome_zarr(data, output_path, channel_names, image_name="default",
                   scale_num_levels=3, scale_factor=0.5, scale_order=0):
    import ipdb
    ipdb.set_trace()
    ...
    for level in range(1, scale_num_levels):
        levels.append(
            zoom(levels[level - 1], scales, order=scale_order)
        )
    ...
    group.attrs["omero"] = OmeZarrWriter.build_ome(
        ...
        channel_names=channel_names,
        channel_colors=[_DEFAULT_ZARR_COLORS[ix]
                        for ix, _ in enumerate(channel_names)],
...
def read_df(path, nrows=None):
    if str(path).endswith("csv"):
        ...
        raise TypeError(
            f"Unknown file extension. Can only read .csv and .parquet")
    return input_df