...
def collect_outputs(path, dataset, order_names, norm):
    run_names_orig = ['2048_dgcnn',
    '2048_ed_m2ae',
    '2048_int_ed_vndgcnn',
    ...
    'classical_image', 
    'so2_image',]
    run_names = ['DGCNN', 'Point M2AE', 'VN-DGCNN int', 'ImageAE', 'SO2 ImageAE', 'ViT', 'ImageAE', 'SO2 ImageAE']
    ...
    df_recon = df_recon.loc[df_recon['split'] == 'test'].groupby('model').mean().reset_index()
    ...
    df_evolve1 = pd.melt(df_evolve, id_vars=['model'], value_vars=['closest_embedding_distance'])
    ...
    df_emissions2 = pd.melt(df_emissions, id_vars=['model'], value_vars=['inference_time'])

    df_list = [df_recon, df_class, df_compact1, df_compact2, df_inv, df_evolve1, df_evolve2, df_emissions1, df_emissions2]
    df = pd.concat(df_list, axis=0).reset_index(drop=True)

    rep_dict_var = {'loss': 'Reconstruction', 'top_1_acc': 'Cell cycle classification', 
                    'compactness': 'Compactness', 'percent_same': 'Outlier Detection', 
                    'rotation_inv_error': 'Rotation Invariance Error', 
                    'closest_embedding_distance': 'Interpolation Embedding Distance', 
                    'energy': 'Shape Evolution Energy', 'emissions': 'Emissions', 
                    'inference_time': 'Inference Time'}
    df['variable'] = df['variable'].replace(rep_dict_var)
    return df

def plot(df, models, title):
    ...
    fig = go.Figure(
        data=[
            go.Scatterpolar(r=all_models[i], theta=categories, fill=fill, name=models[i],opacity=opacity) for i in range(len(all_models))],
        layout=go.Layout(
