...
def run_reconstruction_test(info: Info):
    coords, seg_mem, seg_nuc, seg_str, pilr = compute_pilr_and_save(
        info, return_data=True
    )
    rec_str = cytoparam.morph_representation_on_shape(
        img=seg_mem + seg_nuc, param_img_coords=coords, representation=pilr
    )
    ...
    w.save(
        rec_str,
        infos[0].output_path.replace(".ome.tiff", "_rec.ome.tiff"),
        dim_order="ZYX",
    )
    return
...
if __name__ == "__main__":
    ...
    with ProcessPoolExecutor(multiprocessing.cpu_count()) as executor:
        _ = list(tqdm(executor.map(compute_pilr_and_save, infos), total=len(infos)))
