...
def do_clusterering(
    embeddings, n_clusters, random_state=0, do_cluster_centers=1, n_pca=32
):
    """
    ...
    pca = PCA(n_components=n_pca, svd_solver="arpack", random_state=random_state).fit(
        embeddings
    )
    embeddings_pca_reduced = pca.fit_transform(embeddings)
    cls_gmm = GaussianMixture(n_components=n_clusters, random_state=random_state).fit(
        embeddings_pca_reduced
    )
    labels_gmm = cls_gmm.predict(embeddings_pca_reduced)

    ## kmeans
    cls_kmeans = KMeans(n_clusters=n_clusters, random_state=random_state).fit(
        embeddings
    )
    labels_kmeans = cls_kmeans.labels_

    ## compute  cluster centers
    centers_gmm = pca.inverse_transform(
        cls_gmm.means_
    ...
    scores_kmeans = (
        -np.linalg.norm(kmeans_center_per_label - embeddings.numpy(), ord=2, axis=1)
        ** 2
    )

    return (
        (labels_gmm, labels_kmeans),
        ((pca, cls_gmm), cls_kmeans),
        (centers_gmm, centers_kmeans),
        (scores_gmm, scores_kmeans),
    )
...
def make_sample_grid_for_clustering(
    labels_img,
    data_imgs,
    scores,
    method=None,
    ...
    grid = make_grid(torch.Tensor(sample_imgs), n_examples, pad_value=0.5).moveaxis(
        0, 2
    )
...
def do_sampling(
    l, labels_img, scores, method=None, n_examples=10, stds_filt=1, verbose=True
):
    """
    ...
    # get elements within `stds_filt` of the mean of scores in this cluster
        ...
        scores_in_range = (scores_ >= (mean - std * stds_filt)) & (
            scores_ <= (mean + std * stds_filt)
        )
        idx_samples = idx_samples[scores_in_range]
        n_after = len(idx_samples)
        if verbose:
            print(
                f"STD dev reduction removes {100*(1-n_after/n_before):.0f}% of points"
            )
        ...
        # unfiform but stop early up to the 80th perdentile
            ...
            idx_uniform = np.linspace(
                0, int(len(idx_samples) * 0.8), n_examples
            ).astype(int)
